...
@section('header-card')
    <div class="row">
        <div class="col-9">
            <h4 class="mb-0">Modificar Departamento</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb p-0">
                    <li class="breadcrumb-item"><a href="{{ route('home') }}">Home</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('admin.departament.index') }}">Departamentos</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Modificar departamento</li>
                </ol>
            </nav>
        </div>
...
@section('title-card', 'Modificar Departamento')

@section('content')
    @inject('departaments', 'App\Services\DepartamentService')
    <div class="row justify-content-center">
        <div class="col-8">
            <form method="POST" name="departament" action="{{ route('admin.departament.update',$departament) }}">
                @csrf
                @method('PUT')
                <div class="form-group row">
                    ...
                    <div class="col-md-6">
                        <input id="departamento" type="text" class="form-control @error('departamento') is-invalid @enderror" name="departamento" value="{{ old('departamento',$departament->departamento) }}" autocomplete="departamento" autofocus>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-6">
                        <select id="personal_id" class="form-control @error('personal_id') is-invalid @enderror" name="personal_id">
                            ...
                            <option value="{{ $index }}" {{ old('personal_id',$departament->personal_id) == $index ? 'selected' : '' }}>
                                {{ $personal }}
                            </option>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-md-6">
                        <select id="cargo" class="form-control @error('cargo') is-invalid @enderror" name="cargo">
                            ...
                            <option value="{{ $index }}" {{ old('cargo',$departament->cargo) == $cargo ? 'selected' : '' }}
                            @if( old('cargo') == $index )
                            {{ 'selected' }}
                            @endif
                            >
