...
@section('header-card')
    <div class="row">
        ...
        <div class="col-0 text-right">
            <a href="{{ route('admin.activity.create') }}" class="btn btn-outline-primary">Nueva Actividad</a>
            <a href="{{ route('admin.acom.index') }}" class="btn btn-outline-primary">Volver  ACOMS</a>
        </div>
    </div>
@endsection

@section('title-card', 'Actividades')

@section('content')
    <form id="form-buscar" method="GET" action="{{ route('admin.activity.index') }}">
                    ...
                    <tbody>
                        @foreach ($activities as $activity)
                            <tr>
                                ...
                                <td>
                                    {{ $activity->acom->nombre.'-'.$activity->acom->descripcion }}
                                </td>
                                <td>{{ $activity->creditos }}</td>
                                <td>{{ $activity->duracion }}</td>
                                <td>{{ $activity->descripcion }}</td>
                                <td class="text-center" style="width: 150px">
                                        <form class="form-eliminar m-0 p-0" method="POST" action="{{ route('admin.activity.destroy', $activity) }}">
                                            @csrf
                                            @method('DELETE')

                                            <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                                <a href="{{ route('admin.activity.edit', $activity) }}" class="btn btn-outline-primary">Modificar</a>
