...
@section('header-card')
    <div class="row">
        ...
        <div class="col-3 text-right">
            <a href="{{ route('admin.role.create') }}" class="btn btn-outline-primary">Nuevo rol</a>
        </div>
    </div>
@endsection

@section('title-card', 'Roles')

@section('content')
    <form id="form-buscar" method="GET" action="{{ route('admin.role.index') }}">
        ...
        <div class="col-12">
                        ...
                        @foreach ($roles as $role)
                            <tr>
                                ...
                                <td class="text-center" style="width: 150px">
                                        <form class="form-eliminar m-0 p-0" method="POST" action="{{ route('admin.role.destroy', $role) }}">
                                            @csrf
                                            @method('DELETE')

                                            <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                                <a href="{{ route('admin.role.edit', $role) }}" class="btn btn-outline-primary">Modificar</a>
            ...
            <div class="row">
                ...
                <div class="col-6 text-right">
                    <p class="m-0 mt-2">Página {{ $roles->currentPage() }} de {{ $roles->lastPage() }} </p>
                </div>
            </div>
        </div>
    </div>
