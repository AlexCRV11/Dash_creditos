...
@section('header-card')
    <div class="row">
        ...
        <div class="col-4 text-right">
            <a href="{{ route('admin.personal.create') }}" class="btn btn-outline-primary">Nuevo Profesor</a>
            <a href="{{ route('admin.profession.index') }}" class="btn btn-outline-primary">Listado de profesiones</a>
        </div>
    </div>
@endsection

@section('title-card', 'Profesores')

@section('content')
    <form id="form-buscar" method="GET" action="{{ route('admin.personal.index') }}">
        ...
        <div class="col-12">
                    ...
                    <tbody>
                        @foreach ($personals as $personal)
                            <tr>
                                ...
                                <td>{{ $personal->profession->profesion }}</td>
                                <td class="text-center" style="width: 150px">
                                        <form class="form-eliminar m-0 p-0" method="POST" action="{{ route('admin.personal.destroy', $personal) }}">
                                            @csrf
                                            @method('DELETE')

                                            <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                                                <a href="{{ route('admin.personal.edit', $personal) }}" class="btn btn-outline-primary">Modificar</a>
            ...
            <div class="row">
                ...
                <div class="col-6 text-right">
                    <p class="m-0 mt-2">Página {{ $personals->currentPage() }} de {{ $personals->lastPage() }} </p>
                </div>
            </div>
        </div>
    </div>
